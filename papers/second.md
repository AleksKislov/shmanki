...
В базовом варианте реализации для всех карточек предыдущего шага используются равные веса $w_p = 1 / |Pred(c)|$. Величина $H_c \in [0, 1]$ интерпретируется как степень опоры текущей карточки на уже сформированную предметную схему: чем выше значение $H_c$, тем лучше усвоены необходимые предпосылки.
