              ...
              <div class="card-body gap-5">
                <div class="flex items-center justify-between gap-3">
                  <span class="text-xs font-semibold uppercase tracking-[0.24em] text-primary/80">
                    {t(locale.value, "home.panelLabel")}
                  </span>
                  <div class="badge badge-secondary badge-soft">{t(locale.value, "home.panelLocale")}</div>
                </div>

                <div class="stats stats-vertical border border-base-300 bg-base-100">
                  <div class="stat px-5 py-4">
                    <div class="stat-title">{t(locale.value, "home.stat1.title")}</div>
                    <div class="stat-value text-2xl">{t(locale.value, "home.stat1.value")}</div>
                    <div class="stat-desc">{t(locale.value, "home.stat1.desc")}</div>
                  </div>
                  <div class="stat px-5 py-4">
                    <div class="stat-title">{t(locale.value, "home.stat2.title")}</div>
                    <div class="stat-value text-2xl">{t(locale.value, "home.stat2.value")}</div>
                    <div class="stat-desc">{t(locale.value, "home.stat2.desc")}</div>
                  </div>
                </div>
            ...
            <div class="card-body gap-4">
              <div class="flex items-center justify-between gap-3">
                ...
                <div class="badge badge-ghost">
                  {t(locale.value, "home.stepBadge")} {index + 1}
                </div>
              </div>
              <h2 class="font-display card-title text-2xl">
                {t(locale.value, `${featureKey}.title`)}
              </h2>
              <p class="leading-7 text-base-content/70">
                {t(locale.value, `${featureKey}.text`)}
              </p>
            </div>
          </article>
        ))}
      ...
      <section class="grid gap-4 lg:grid-cols-[2fr_1fr]">
        <article class="card border border-base-300 bg-neutral text-neutral-content shadow-sm">
          <div class="card-body gap-5 p-8">
            <p class="badge badge-outline badge-lg">{t(locale.value, "home.noteTitle")}</p>
            <p class="font-display text-3xl leading-tight sm:text-4xl">
              {t(locale.value, "home.noteText")}
            </p>
          </div>
        </article>
        ...
        <article class="card border border-base-300 bg-base-100 shadow-sm">
          <div class="card-body p-7">
            <h2 class="card-title">{t(locale.value, "home.listTitle")}</h2>
            <ul class="menu rounded-box bg-base-200">
              {listKeys.map((listKey) => (
                <li key={listKey}>
                  <span>{t(locale.value, listKey)}</span>
                </li>
              ))}
            </ul>
          </div>
