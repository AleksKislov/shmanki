...
export default component$(() => {
  ...
  const handleSubmit$ = $(async () => {
    ...
    try {
      ...
      error.value = e instanceof Error ? e.message : t(locale.value, "auth.login.error");
    } finally {
      loading.value = false;
    }
  });
    ...
    <div class="flex min-h-[70vh] items-center justify-center">
      <div class="card w-full max-w-md border border-base-300 bg-base-100 shadow-sm">
        <div class="card-body gap-5">
          <h1 class="card-title text-2xl">{t(locale.value, "auth.login.title")}</h1>
          ...
          >
            {loading.value ? (
              ...
              t(locale.value, "auth.login.submit")
            )}
          </button>

          <p class="text-center text-sm text-base-content/70">
            {t(locale.value, "auth.login.noAccount")}{" "}
            <Link class="link link-primary" href="/auth/register">
              {t(locale.value, "auth.login.register")}
            </Link>
          </p>
        </div>
      </div>
